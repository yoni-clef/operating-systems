...
public class GUI
{
    ...
    public GUI()
    {
        ...
        addBtn.setBackground(Color.GREEN); // Set button background color
        addBtn.setForeground(Color.BLACK); // Set button text color
        ...
        removeBtn.setBackground(Color.RED); // Set button background color
        removeBtn.setForeground(Color.WHITE); // Set button text color
        ...
        wtLabel.setForeground(Color.BLUE); // Set label text color
        tatLabel = new JLabel("Average Turn Around Time:");
        tatLabel.setBounds(25, 450, 180, 25);
        tatLabel.setForeground(Color.BLUE); // Set label text color
        wtResultLabel = new JLabel();
        wtResultLabel.setBounds(215, 425, 180, 25);
        wtResultLabel.setForeground(Color.MAGENTA); // Set result label text color
        tatResultLabel = new JLabel();
        tatResultLabel.setBounds(215, 450, 180, 25);
        tatResultLabel.setForeground(Color.MAGENTA); // Set result label text color
        
        option = new JComboBox<>(new String[]{ "SRT"});
        ...
        computeBtn.setBackground(Color.ORANGE); // Set button background color
        computeBtn.setForeground(Color.BLACK); // Set button text color
        
        mainPanel = new JPanel(null);
        mainPanel.setPreferredSize(new Dimension(500, 500));
        mainPanel.setBackground(Color.white); // Set main panel background color
        ...
        protected void paintComponent(Graphics g)
        {
            ...
            if (timeline != null)
            {
                for (int i = 0; i < timeline.size(); i++)
                {
                    ...
                    g.setColor(Color.BLACK); // Set color for rectangle border
                    g.drawRect(x, y, 30, 30);
                    g.setFont(new Font("Segoe UI", Font.BOLD, 13));
                    g.setColor(Color.BLUE); // Set color for process name
                    g.drawString(event.getProcessName(), x + 10, y + 20);
                    g.setFont(new Font("Segoe UI", Font.PLAIN, 11));
                    g.setColor(Color.RED); // Set color for start time
                    g.drawString(Integer.toString(event.getStartTime()), x - 5, y + 45);
                    
                    if (i == timeline.size() - 1)
                    {
                        g.setColor(Color.RED); // Set color for finish time
